document.getElementById('sortOptions').addEventListener('change', function() {
    ...
    if (sortBy === 'price') {
        sortedItems = items.sort((a, b) => {
            const priceA = parseFloat(a.querySelector('.card-body p').innerText.replace('₫', '').replace(/,/g, ''));
            const priceB = parseFloat(b.querySelector('.card-body p').innerText.replace('₫', '').replace(/,/g, ''));
